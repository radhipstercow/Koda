...
final class DrinkCell: UICollectionViewCell {
    ...
    private(set) lazy var nameLabel: UILabel = self.makeLabel(color: POSColor.darkText)
    ...
    func configure(with drink: posItem) {
        nameLabel.text = drink.itemName
        if let urlString = drink.itemImageUrl {
    ...
    private func makeImageView() -> UIImageView {
        ...
        imageView.backgroundColor = POSColor.lightText
    ...
    private func makeLabel(color: UIColor) -> UILabel {
        let label = UILabel()
        label.font = .systemFont(ofSize: 19.0, weight: UIFont.Weight.medium)
